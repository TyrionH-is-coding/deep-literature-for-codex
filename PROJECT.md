...
**007 新隔离实例已就绪，论文已入库，当前等待用户本人登录 GPT。** [007 任务卡](docs/project/tasks/V02-007.md)固定同一 dev.5 制品，目标为入库 → 获取 PDF → MinerU → 实际 GPT 模型 → Reader → 记录保存与重启读回。复用已有恢复和回归证据，不重复无变化源码全集。[当前交接](docs/project/evidence/V02-007-handoff.md)保存实例、论文和分类会话身份；007尚未通过。

用户指定的 CD4+ Perturb-seq 论文已查到 Cell 正式版（DOI `10.1016/j.cell.2026.08.002`）；正式全文站出现访问验证，公开 bioRxiv v1（63页）已下载并以预印本题录入库，分类为“CD4 T细胞与免疫调控”。本轮只处理这一篇，不自动增加付费重试。用户接下来在[新实例登录页](http://127.0.0.1:49686/api/codex-oauth/ui)本人授权 GPT，随后选择实际可用型号并配置 MinerU Key，最后判断内容质量。此URL绑定本次启动，重启须重新核对CLI返回。完整精读尚未提交，未调用 GPT/MinerU；定时循环保持暂停至本地配置可继续。
