...
**007 已备妥验收卡，等待用户提供论文和真实服务条件；自动循环已暂停。** [007 任务卡](docs/project/tasks/V02-007.md)将固定同一 dev.5 制品，在全新隔离实例完成一篇真实论文：入库 → 获取 PDF → MinerU → 实际模型 → Reader → 记录保存与重启读回。复用已有恢复和回归证据，不重复无变化源码全集。

用户需要提供论文 DOI/链接及本次服务用量范围，并在隔离实例本人配置模型登录/选择与 MinerU Key，最后判断内容质量。密钥不进入聊天或报告；外层 Codex 登录不代表工作台已登录。尚未启动真实解析或模型调用。
