...
用户已授权[自动总控循环](docs/project/auto-loop.md)：每 10 分钟按验收结果自动推进，最多两个独立开发任务并行，合入串行。003A 和 002B 已复核集成到引擎 `4e5e6b9`；组合源码的 56 项 Excel 测试、构建、导航合同和 Excel 接口复验通过，F3 关闭。见[组合复核](docs/project/evidence/V02-003A-review.md)。后继为 003B（内部候选包安装与 F1 原场景复验）和独立的 006B（固定 DSH 的会话恢复能力取证）。F1 尚待安装制品复测，F2 尚无完整恢复实现；实际派发状态以任务台账为准。
