...
自动循环并行推进 **004B 最小恢复修复** 与 **004C 取消合同诊断/设计**：前者只改 A 同源 generation 恢复，后者只写 B 专属文档/探针并读取冻结 A，实例与接口互不影响。源码修复后仍需新制品安装复验；当前产品 pins 仍是已安装验证的 dev.2。实际派发状态以[台账](docs/project/tasks.json)为准。
