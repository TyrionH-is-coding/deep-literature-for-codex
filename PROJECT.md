...
用户已授权[自动总控循环](docs/project/auto-loop.md)：每 10 分钟按验收结果自动推进，最多两个独立开发任务并行，合入串行。003A 和 002B 已复核集成到引擎 `4e5e6b9`；组合源码的 56 项 Excel 测试、构建、导航合同和 Excel 接口复验通过，F3 关闭。见[组合复核](docs/project/evidence/V02-003A-review.md)。003B 的引擎候选全套回归通过（515 Python pass / 3 skip、45 条 Node 命令），但 B 打包被 catalog/pins 身份不一致拦住，安装未执行。该关联元数据已由 003C 补齐，dev.1 真实安装的数据保护复验通过（14 项 conflict、12 项 smoke，另保留 F2 known_gap）。但文献详情页没有显示 HTTP 已提供的冲突原因/字段，记录 F4，交 003D 做最小 UI 修复和新候选复验。B 候选尚未合入总控，003/004 依赖门槛未解除。006B 已复核集成，仅证明固定 DSH 文本历史/队列子集的迁移可行性，尚无产品级持久禁重放门禁。F1/F2 继续开放；实际派发状态以任务台账为准。
