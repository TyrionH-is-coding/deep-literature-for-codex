...
for (const file of ['README.md', 'START_HERE.md', 'AGENTS.md', 'LICENSE', 'THIRD_PARTY_NOTICES.md', 'package.json', 'package-lock.json', 'install.ps1', 'workbench.ps1', 'uninstall.ps1', 'install.sh', 'workbench.sh', 'uninstall.sh']) await copy(file);
...
for (const folder of ['oauth/tests', 'docs/modules']) await fs.cp(path.join(source, folder), path.join(packageRoot, folder), { recursive: true });
