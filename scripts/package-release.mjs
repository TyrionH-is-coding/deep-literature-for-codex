...
import { checkModules, moduleSnapshot } from './module-graph.mjs';
...
for (const file of ['README.md', 'START_HERE.md', 'AGENTS.md', 'PROJECT.md', 'LICENSE', 'THIRD_PARTY_NOTICES.md', 'package.json', 'package-lock.json', 'install.ps1', 'workbench.ps1', 'uninstall.ps1', 'install.sh', 'workbench.sh', 'uninstall.sh']) await copy(file);
...
for (const file of ['LICENSE', 'NOTICE']) await copy('oauth/vendor/openai-codex/' + file);
for (const folder of ['oauth/tests', 'docs/modules', 'docs/project']) await fs.cp(path.join(source, folder), path.join(packageRoot, folder), { recursive: true });
...
const moduleInventory = { ...await moduleSnapshot(packageRoot), scope: 'packaged-files' };
await writeJson(path.join(packageRoot, 'BUILD-MANIFEST.json'), { schema: 1, product: 'codex-scientific-reading', version: VERSION,
  displayName: 'Deep Literature for Codex', channel: pins.channel, sourceCommit, pluginSourceCommit: pins.plugin.sourceCommit, pins, modules: moduleInventory, files });
