...
function probe(mode){const args=['-I','-X','utf8',path.join(import.meta.dirname,'fixtures/v02-004d/conflict-probe.py'),root,mode];const r=spawnSync(installed.python,args,{cwd:root,env:isolatedEnvironment(root,process.env,installed),encoding:'utf8',windowsHide:true,maxBuffer:16*1024*1024});report.commands.push({command:installed.python,args,code:r.status,stdout:r.stdout,stderr:r.stderr});assert.equal(r.status,0,r.stderr);return JSON.parse(r.stdout);}
let running;
try{
 await stop(root);
