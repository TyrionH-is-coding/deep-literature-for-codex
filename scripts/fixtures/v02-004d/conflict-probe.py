"""V02-004D installed-only CLI/worker assertions; all data synthetic."""
...
assert (root/'.v02-004d-test-instance').read_text()=='synthetic-only'
...
def seed(data):
 lib=LibraryService(data);ids=[lib.ingest(PaperMetadata(title='V02-004D synthetic '+str(i),doi='10.9999/v3d-'+str(i)))['paper_id'] for i in range(2)];lib.close()
...
if mode=='cases':
 ...
 assert cli(data,'xlsx-refresh')['status']=='success'
 lib=LibraryService(data);other=lib.ingest(PaperMetadata(title='V02-004D unrelated row',doi='10.9999/v3d-unrelated'))['paper_id'];lib.close()
 assert cli(data,'xlsx-refresh')['status']=='success'
 ...
 (root/'v3d-conflict.json').write_text(json.dumps(state),encoding='utf-8');print(json.dumps(state))
