...
p=subprocess.run([sys.executable,'-I','-X','utf8','-m','scientific_reading.worker','--data-root',str(data),'--job-id',job],capture_output=True,text=True,encoding='utf-8',creationflags=getattr(subprocess,'CREATE_NO_WINDOW',0));assert p.returncode==2,(p.stdout,p.stderr)
