"""V02-003D installed-only CLI/worker assertions; all data synthetic."""
...
def seed(data):
 lib=LibraryService(data);ids=[lib.ingest(PaperMetadata(title='V02-003D synthetic '+str(i),doi='10.9999/v3d-'+str(i)))['paper_id'] for i in range(2)];lib.close()
...
if mode=='cases':
 ...
 assert cli(data,'xlsx-refresh')['status']=='success'
 lib=LibraryService(data);other=lib.ingest(PaperMetadata(title='V02-003D unrelated row',doi='10.9999/v3d-unrelated'))['paper_id'];lib.close()
 assert cli(data,'xlsx-refresh')['status']=='success'
