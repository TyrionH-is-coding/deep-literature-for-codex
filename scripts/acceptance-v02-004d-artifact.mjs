...
const p=JSON.parse(await fs.readFile('docs/project/evidence/V02-004D/package.txt','utf8'));
...
await fs.writeFile('docs/project/evidence/V02-004D/artifact.json',JSON.stringify({release,verify,manifestSource:manifest.sourceCommit,manifestEngineSource:manifest.pluginSourceCommit,archive,archiveSha256:await sha(archive),wheel: path.join(dir,'package/dist/python',wheel),wheelSha256:await sha(path.join(dir,'package/dist/python',wheel)),manifestSha256:await sha(path.join(p.extractedRoot,'BUILD-MANIFEST.json'))},null,2));
