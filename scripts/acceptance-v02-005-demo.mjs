...
import {root,out,installed,read,start,stop,status,action,gate,py,engine,save} from './fixtures/v02-005/common.mjs';
...
try {
 ...
 const folder=await record('create category',await action('folder_create',{name:'V02-005 synthetic '+mode+' '+caseName}));
 report.folderId=folder.folder_id;
 await record('bind category',await action('bind',{folderId:report.folderId}));
 const metadata=mode==='failure'?{...payload.metadata,title:payload.metadata.title+' - V02-005 provider failure'}:payload.metadata;
 ...
 await fs.writeFile(path.join(root,'v005-provider',caseName+'.json'),JSON.stringify(providerPayload));
 await fs.writeFile(path.join(root,'v005-provider/mode.json'),JSON.stringify({case:caseName,fail_once:mode==='failure'}));
 task=await action('submit',{folderId:report.folderId,paperId:report.paperId,idempotencyKey:'v005-'+mode+'-'+caseName,runAgent:false});
 ...
 const pdf=path.join(root,'v005-provider',caseName+'.pdf');
 report.sourceSha256=createHash('sha256').update(await fs.readFile(pdf)).digest('hex');
 await record('attach PDF',await action('attach',{taskId:task.taskId,idempotencyKey:'v005-attach-'+mode+'-'+caseName,pdf,sourceType:'manual'}));
 ...
 if(mode==='failure') {
  ...
  await record('explicit resumeStopped',await action('resume',{taskId:task.taskId,idempotencyKey:'v005-explicit-safe-recovery-r'+canceled.control.revision,resumeStopped:true,expectedRevision:canceled.control.revision,input:{}}));
  ...
  while(task.job.status==='waiting_agent') {
   ...
   await record('submit synthetic '+detail.reason_code,await action('resume',{taskId:task.taskId,idempotencyKey:'v005-'+caseName+'-v3-gate-'+n,input}));
