...
assert.equal(await fs.readFile(path.join(root, '.v02-001-test-instance'), 'utf8'), 'synthetic-only');
...
assert.equal(installed.pins.plugin.sha256, '318814ec542de94a47cd2855b21d0f7af9778b22b283d3995d0c499a3465e2fd');
...
const probe = async mode => JSON.parse(await run(installed.python,['-I','-X','utf8',path.join(here,'fixtures','v02-001','engine-probe.py'),root,mode]));
...
try {
  ...
  if(info) {
    ...
    report.checks.push({name:'whole instance recovery includes native sessions and handoff',status:report.nativeSessionCreated&&report.handoffExists?'failed':'unverified',reason:'library backup scope excludes state/dsh-home and state/handoff.json; library-only restore is not whole-instance restore'});
  }
} finally {
  ...
  process.exitCode=report.checks.some(c=>c.status!=='passed')?1:0;
}
