...
const cwd=process.cwd(), p=JSON.parse(await fs.readFile('docs/project/evidence/V02-003D/package.txt','utf8'));
...
await initializeRoot(root); await fs.writeFile(path.join(root,'.v02-003d-test-instance'),'synthetic-only');
...
await fs.writeFile('docs/project/evidence/V02-003D/cache-copy.json',JSON.stringify({root,extractedRoot:p.extractedRoot,records},null,2));
